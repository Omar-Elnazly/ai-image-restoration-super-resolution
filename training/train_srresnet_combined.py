"""
SRResNet Combined Training — DIV2K + CelebA

Loads pretrained DIV2K SRResNet weights and fine-tunes
on combined DIV2K + CelebA dataset.

Original checkpoints are never touched.

Run:
    python training/train_srresnet_combined.py
"""
...
def train_srresnet_combined(
    config_path: str = "configs/config_celeba.yaml",
    pretrained_path: str = "checkpoints/srresnet/srresnet_best.pth",
):
    ...
    print(f"\n{'='*50}")
    print(f"  SRResNet Combined Training (DIV2K + CelebA)")
    ...
    best_psnr = 0.0
    logger    = TensorBoardLogger(paths['logs'], 'srresnet_combined')
    ...
    for epoch in range(train_cfg['epochs']):
        ...
        for batch_idx, (lr_batch, hr_batch) in enumerate(progress):
            ...
            # SRResNet takes raw LR directly
        ...
        if val_avg['psnr'] > best_psnr:
            ...
            save_checkpoint(
                ...
                os.path.join(checkpoint_dir, 'srresnet_combined_best.pth'), scaler
            )
        ...
        if (epoch + 1) % train_cfg['save_interval'] == 0:
            save_checkpoint(
                ...
                os.path.join(
                    checkpoint_dir,
                    f'srresnet_combined_epoch_{epoch+1:04d}.pth'
                ), scaler
            )

    logger.close()
    print(f"\n  Combined SRResNet training complete.")
    print(f"  Best PSNR : {best_psnr:.2f} dB")
    print(f"  Saved to  : {checkpoint_dir}/srresnet_combined_best.pth")
    print(f"  Original  : checkpoints/srresnet/srresnet_best.pth (untouched)")


if __name__ == "__main__":
    train_srresnet_combined()